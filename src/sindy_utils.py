...
def sindy_library_order2(X, dX, poly_order, include_sine=False, include_cosine=False):
    m,n = X.shape
    l = library_size(2*n, poly_order, include_sine, include_cosine, True)
    library = np.ones((m,l))
    index = 1
...
def sindy_simulate_order2(x0, dx0, t, Xi, poly_order, include_sine, include_cosine=False):
    m = t.size
    n = 2*x0.size
    l = Xi.shape[0]
    ...
    x = sindy_simulate(np.concatenate((x0,dx0)), t, Xi_order1, poly_order, include_sine, include_cosine)
    return x
